...
界面下方「🎬 二次创作」面板，从已备份的视频库随机合成竖屏短片：

1. 随机选 1 个视频，在 **50% 位置抽一帧**关键帧
2. 关键帧加**白色细边 + 阴影**，缩到约 **300px 宽**，居中叠加
3. 随机取 **2 段视频，各抽 5 秒**，上下叠加铺满竖屏（1080×1920）
4. 合成 5 秒成品，存到 `outputs/`，界面里直接预览 / 下载

填「数量」可一次批量生成多个（每个都随机，互不相同）。
